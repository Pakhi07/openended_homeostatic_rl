...
# use this param to control the freq. of video recording
LONG_EPISODE = 100
...
class StatsRecorder(gymnasium.Wrapper):
...
class VideoRecorder(gymnasium.Wrapper):
  ...
  def step(self, action):
    ...
    if done and self._env.is_long_episode:
      self._save()
    return obs, reward, done, truncated, info
...
class EpisodeRecorder(gymnasium.Wrapper):
  ...
  def step(self, action):
    ...
    obs, reward, done, truncated, info = self._env.step(action)
    transition = {
        'action': action, 'image': obs, 'reward': reward, 'done': done, 'truncated': truncated,
    }
...
class EpisodeName:
  ...
  def is_long_episode(self):
    return self._length > LONG_EPISODE
